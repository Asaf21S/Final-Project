...
def show_features(data_folder):
    # import the data from the csv files
    ideal_th = np.array([])
    features = np.zeros((0, 37))
    print(ideal_th.shape, ideal_th)
    print(features.shape, features)
    for file_name in listdir(data_folder):
        if file_name.endswith(".csv") and file_name.startswith("Training_Data"):
            # names = np.loadtxt(data_folder + "\\" + file_name, dtype='str', delimiter=',', usecols=0)
            file_ideal_th = np.loadtxt(data_folder + "\\" + file_name, dtype='float', delimiter=',', usecols=1)
            file_features = np.loadtxt(data_folder + "\\" + file_name, dtype='float', delimiter=',', usecols=tuple(range(2, 39)))
            print(file_name, file_ideal_th.shape, file_features.shape)
            ideal_th = np.concatenate((ideal_th, file_ideal_th))
            features = np.concatenate((features, file_features))
    print(ideal_th.shape, features.shape)
    ...
    for feature in range(features.shape[1]):
        plt.figure()
        plt.scatter(features[:, feature], ideal_th[:], alpha=0.1)
...
if __name__ == '__main__':
    ...
    show_features(model_data_folder)
